...
{
	int x, y, n;
	uint8_t* image_data = stbi_load(input_file_path.c_str(), &x, &y, &n, 1);
	...
	SquashHeader header{static_cast<uint32_t>(x), static_cast<uint32_t>(y), ImageChannels::Grey};
	...
	for (int i = 0; i < x_blocks; i++)
	{
		for (int j = 0; j < y_blocks; j++)
		{
			auto block = math::Matrix<priv::N, priv::N, uint8_t>::FromFunction(
				[&image_data, i, j, x, y](size_t k, size_t l)->uint8_t {
					if (i + k >= x) return 128;
					if (j + l >= y) return 128;

					// TODO: something's suspicious here...
					return image_data[(x * ((priv::N * j) + l)) + (priv::N * i) + k];
				});
